...
| `ending_burned` | ~~"You board the 02:10 alone."~~ **[EE-4 VIOLATION — fixed]** Original prose asserted player boards alone as fact. Every forward path to this ending sets `companion = 'dragomir'` (e.g. `take_decoy_leave` in `node_sperl_press_film` sets `companion = dragomir` while also yielding `dragomir_trust < 3`). The audit previously argued trust < 3 prevented `companion` from being set — this was wrong. **Fix:** prose now reads "Whether she is sitting across from you now … or whether the wet streets swallowed her … the failure is the same." — companion-agnostic conditional phrasing matching `ending_missed`'s pattern. No unbacked companion claim remains. | `companion = dragomir` is set on every reaching path; prose now makes no factual claim about companion presence or absence | All Sperl exit choices toward the crossroads (`walk_with_me`, `get_moving`, `tell_canal_handoff`, `take_decoy_leave`, `into_the_rain`) set `companion = dragomir` | `"reaches Smoke on the Embankment (burned)"` (prose rewritten — see EE-4 fix pass) |
...
**Pass (after fix).** Every load-bearing prose claim in the five endings and six state-gated variant nodes (`node_sperl_trust`, `node_get_real_film`, `node_handoff_witnessed`, `node_canal_drop`, `node_volkov` + `take_deal` condition, `node_volkov_truth`) maps to a flag that is set on the path that reaches it.

**One violation was found and fixed.** `ending_burned` ("Smoke on the Embankment") originally narrated "You board the 02:10 alone." as fact. This was an EE-4 violation: every forward path that reaches this ending passes through a Sperl exit choice that sets `companion = 'dragomir'` (e.g. `take_decoy_leave` in `node_sperl_press_film` sets companion while still yielding `dragomir_trust < 3`). The prior audit argument — that trust < 3 prevents `companion` from being set — was wrong. The choices that set `companion = dragomir` do not require elevated trust; they require only leaving Café Sperl toward the crossroads, which every burned path does.

**Fix applied:** `ending_burned` prose was rewritten to use companion-agnostic conditional phrasing — "Whether she is sitting across from you now … or whether the wet streets swallowed her … the failure is the same" — mirroring the approach already used correctly in `ending_missed`. The state-backed claims remain intact: the decoy canister (`has_real_microfilm = false`), broken trust (`dragomir_trust < 3`), and Volkov's closed net (`volkov_suspicion >= 4`) are all genuine resolver conditions on the burned path and are still narrated as fact. No companion claim (present or absent) is asserted without backing.
